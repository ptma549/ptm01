...
export class JobService {
  ...
  protected convertDateFromClient(job: IJob): IJob {
    const copy: IJob = Object.assign({}, job, {
      completed: job.completed && job.completed.isValid() ? job.completed.toJSON() : undefined,
      reportedOn: job.reportedOn && job.reportedOn.isValid() ? job.reportedOn.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.reportedOn = res.body.reportedOn ? moment(res.body.reportedOn) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((job: IJob) => {
        job.completed = job.completed ? moment(job.completed) : undefined;
        job.reportedOn = job.reportedOn ? moment(job.reportedOn) : undefined;
      });
    }
    return res;
  }

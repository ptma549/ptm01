...
public interface JobRepository extends JpaRepository<Job, Long> {
    ...
    @Query(value = "select distinct job from Job job left join fetch job.clients",
        countQuery = "select count(distinct job) from Job job")
    Page<Job> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct job from Job job left join fetch job.clients")
    List<Job> findAllWithEagerRelationships();

    @Query("select job from Job job left join fetch job.clients where job.id =:id")
